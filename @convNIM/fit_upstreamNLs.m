function cnim = fit_upstreamNLs(cnim, Robs, Xstims, varargin)
...
poss_targets = find(strcmp(cnim.get_NLtypes,'nonpar'))'; %can only fit subunits with nonpar NLs
...
defaults.fit_spk_hist = cnim.spk_hist.spkhstlen > 0; %default is fit the spkNL filter if it exists
...
filtered_kernels = cell(Nfit_subs+1,1); % store filtered conv kernels; store tbspaces in last cell for offsets
tbspaces = zeros(Nfit_subs,1);
for i = 1:Nfit_subs
	indx = fit_subs(i);
	if ~isempty(cnim.conv_params(cnim.subunits(indx).conv_targ).tent_spacing)
		...
		for nn = 1:length(tent_filter)
			filtered_kernel((0:num_lags-1)*tbspace+nn) = filtered_kernel((0:num_lags-1)*tbspace+nn) + tent_filter(nn) * cnim.subunits(indx).convK;
		end
		filtered_kernels{i} = filtered_kernel;
		tbspaces(i) = tbspace;
	else
		filtered_kernels{i} = cnim.subunits(indx).convK;
		tbspaces(i) = 1;
	end
end
...
n_TBs = arrayfun(@(x) length(x.NLnonpar.TBx),cnim.subunits(fit_subs));  %get the number of TBs for each subunit
assert(length(unique(n_TBs)) == 1,'Have to have same number of tent-bases for each subunit'); 
n_TBs = unique(n_TBs);

nontarg_g = cnim.conv_process_stimulus(Xstims,non_fit_subs,gain_funs); %get output of nontarget subunits
...
for ii = 1:Nfit_subs %for each module
	cur_sub = fit_subs(ii);
	gint = Xstims{cnim.subunits(cur_sub).Xtarg}*cnim.subunits(cur_sub).filtK;
	% The output of the current model's internal filter projected onto the tent basis representation
	if isempty(gain_funs)
		tbf_out = cnim.subunits(cur_sub).weight * cnim.subunits(cur_sub).tb_rep(gint);
	else
		tbf_out = cnim.subunits(cur_sub).weight * bsxfun(@times,cnim.subunits(cur_sub).tb_rep(gint),gain_funs(:,cur_sub));
	end
...
init_params = [];
for imod = fit_subs
	init_params = [init_params; cnim.subunits(imod).NLnonpar.TBy']; %compile TB parameters 
end
...
lambda_nl = cnim.get_reg_lambdas('subs',fit_subs,'nld2');
if any(lambda_nl > 0)
...
A = []; b = [];
if any(arrayfun(@(x) x.NLnonpar.TBparams.NLmon,cnim.subunits(fit_subs)) ~= 0) %if any of our target nonpar subunits have a monotonicity constraint
	...
	for ii = 1:Nfit_subs
		...
		if cnim.subunits(fit_subs(ii)).NLnonpar.TBparams.NLmon ~= 0
			for jj = 1:length(cur_range)-1 %create constraint matrix
				cur_vec = zvec;
				cur_vec(cur_range([jj jj + 1])) = cnim.subunits(fit_subs(ii)).NLnonpar.TBparams.NLmon*[1 -1];
				A = cat(1,A,cur_vec);
			end
		end
		% Constrain the 0-coefficient to be 0
		[~,zp] = find(cnim.subunits(fit_subs(ii)).NLnonpar.TBx == 0);
...
optim_params = cnim.set_optim_params( optimizer, optim_params, parsed_options.silent );
...
opt_fun = @(K) internal_LL_NLs(cnim, K, filtered_kernels, Robs, XNL, Xspkhst, nontarg_g, Tmat, fit_opts);
...
for ii = 1:Nfit_subs
	...
	if parsed_options.rescale_nls %rescale so that the std dev of the subunit output is conserved
		thisnl = thisnl*cnim.subunits(fit_subs(ii)).scale/cur_std;
	else
		cnim.subunits(fit_subs(ii)).scale = cur_std; %otherwise adjust the model output std dev
	end
	cnim.subunits(fit_subs(ii)).NLnonpar.TBy = thisnl';
	nlmat_resc(:,ii) = thisnl';
	cnim.subunits(fit_subs(ii)).TBy_deriv = cnim.subunits(fit_subs(ii)).get_TB_derivative();
end
if parsed_options.fit_spk_hist
	cnim.spk_hist.coefs = params(Nfit_subs*n_TBs + (1:spkhstlen));
end
...
if parsed_options.rescale_nls
	...
	new_theta = fminunc( @(K) internal_theta_opt(cnim,K,G,Robs), init_theta, opts);
	cnim.spkNL.theta = new_theta;
else
	cnim.spkNL.theta = params(end);
end
...
cnim.fit_props = cur_fit_details;
cnim.fit_history = cat(1,cnim.fit_history,cur_fit_details);
end

%%
function [penLL, penLLgrad] = internal_LL_NLs(nim,params, conv_kernels, Robs, XNL, Xspkhst, nontarg_g, Tmat,fit_opts)
...
% loop through subunits and apply convolution kernel
for i = 1:Nfit_subs
	indxs = ((i-1)*n_TBs + 1):(i*n_TBs);
	sub_out = XNL(:,indxs)*all_TBy(indxs);
	temp = conv(sub_out,conv_kernels{i});
	temp = temp(conv_kernels{end}(i):(conv_kernels{end}(i)+length(G)-1));
	G = G + temp;
end
...
% gradient for tent-basis coeffs
for i = 1:Nfit_subs
	indxs = ((i-1)*n_TBs + 1):(i*n_TBs);
	temp = zeros(length(G)+length(conv_kernels{i})-1,length(indxs));
	for j = 1:length(indxs)
		temp(:,j) = conv(XNL(:,indxs(j)),conv_kernels{i});
	end
	temp = temp(conv_kernels{end}(i):(conv_kernels{end}(i)+length(G)-1),:);
	penLLgrad(indxs) = residual'*temp;
end
